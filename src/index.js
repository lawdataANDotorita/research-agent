...
export default {
	async fetch(request, env, ctx) {
		...
		const bIsFirstMessage = oInputs.isFirstMessage;
...
בפרומפט הבא אתה תקבל את ההקשר: טקסט אחד או יותר, כשכל אחד מהטקסטים מכיל שני שדות עיקריים: שם טקסט ותוכן טקסט. הטקסטים מופרדים ביניהם באמצעות תווי שורה חדשה.
...
מקור יחיד: השב אך ורק על בסיס המידע הקיים בטקסט שסופק.
איסור המצאה (Hallucination): אל תמציא מידע, תסיק מסקנות לא מפורשות, או תשתמש בידע חיצוני.
טיפול בפערים: אם התשובה אינה נמצאת בטקסטים שסופקו, עליך לציין זאת במפורש. (לדוגמה: "התשובה לשאלה זו אינה כלולה בטקסטים שסופקו").
...
בהצלחה.
		...
		if (bIsFirstMessage) {
			sPrompt += `
				הנחיה למתן כותרת לשיחה:
				בסוף התשובה, עליך להוסיף כותרת קצרה ועניינית המתארת את נושא השאלה והתשובה (לדוגמה: "בירור זכויות דייר" או "פרשנות סעיף פיצויים").
				הכותרת צריכה להיות מורכבת  מטקסט בלבד ללא שילוב של אלמנטים של html או אלמנטים של סימון Markdown.
				לפני הכותרת עליך להוסיף את התווים המפרידים *&* בלבד.
